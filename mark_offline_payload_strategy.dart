// mark_offline_payload_strategy.dart
...
class MarkOfflinePayloadStrategy implements PayloadStrategy {
  ...
  Future<void> handle(String endpointId, Map<String, dynamic> data) async {
    ...
    try {
      ...
      print(
        "Found ${existing.length} existing device(s) with uuid $deviceUuid",
      );
      ...
      // Update device status: use ISO8601 string for lastSeen
      await db.update(
        'devices',
        {'isOnline': 0, 'lastSeen': DateTime.now().toIso8601String()},
        where: 'uuid = ?',
        whereArgs: [deviceUuid],
      );
